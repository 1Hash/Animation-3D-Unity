...
public class src_PlayerController : MonoBehaviour
{
    ...
    public src_CameraController cameraController;

    [Header("Movement")]
    public float movementSpeedOffset = 1;
    public float movementSmoothdamp = 0.2f;
    ...
    public float maxFallingMovementSpeed = 10;
    ...
    private void Sprint()
    {
        ...
        if (playerStats.Stamina > (playerStats.MaxSatamina / 4))
        {
            isSprinting = true;
        }
    }
    ...
    private void CalculateSprint()
    {
        ...
        else
        {
            ...
            {
                if (playerStats.Stamina < playerStats.MaxSatamina)
                {
                    playerStats.Stamina += playerStats.StaminaRestore * Time.deltaTime;
                }
                else
                {
                    playerStats.Stamina = playerStats.MaxSatamina;
                }
            }
            else
            {
    ...
    private void CalculateFalling()
    {
        ...
        if (fallingTriggered && isGrounded() && fallingSpeed < -0.9f)
        {
